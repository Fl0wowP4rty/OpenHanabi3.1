...
class PowerShellCodepage {
   private static final Map codePages = new HashMap();

   public static String getCodePageNameByIdetifier(String cpIdentifier) {
      return (String)codePages.get(cpIdentifier);
   }

   public static String getIdentifierByCodePageName(String cpName) {
      if (cpName != null) {
         Iterator var1 = codePages.entrySet().iterator();

         while(var1.hasNext()) {
            Map.Entry codePage = (Map.Entry)var1.next();
            if (((String)codePage.getValue()).toLowerCase().equals(cpName.toLowerCase())) {
               return (String)codePage.getKey();
            }
         }
      }
